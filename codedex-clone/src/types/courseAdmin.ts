...
export interface CourseForAdminList extends Pick<PrismaCourse, 'id' | 'title' | 'slug' | 'created_at' | 'updated_at'> {
  // Añadir conteos si se incluyen desde la API, ej. _count?: { chapters: number }
}

// Para el formulario de creación/edición
export interface CourseFormData extends Omit<PrismaCourse, 'id' | 'created_at' | 'updated_at'> {
