...
const DifficultyBadge: React.FC<{ difficulty: string }> = ({ difficulty }) => {
  ...
  return (
    <div
      className={`${bgColor} text-white px-2 py-1 text-xs rounded-sm font-pixel`}
    >
    ...
    <Card className="bg-codedex-navy border border-codedex-gold/20 overflow-hidden transition-transform hover:translate-y-[-4px] duration-300">
      ...
      <CardContent className="p-6 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <Button
            className="border border-codedex-gold/30 text-codedex-gold hover:bg-codedex-gold/10 hover:text-white"
            asChild
          >
