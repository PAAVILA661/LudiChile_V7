...
export const withAdminApiAuth = (
  handler: (req: AuthenticatedAdminRequest, res: NextApiResponse) => Promise<void> | void
) => {
  return async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
      ...
      //   return res.status(403).json({ message: 'Forbidden: Admin status revoked or user not found' });
      // }
