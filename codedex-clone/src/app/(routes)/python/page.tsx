...
// Define chapter data statically for now
// In a real app, this might come from a CMS or backend
const courseChaptersData = [
  {
    number: 1,
    title: "Hello World",
...
const PythonCoursePage = () => {
  const { user, isLoading } = useAuth(); // 3. Use Auth Hook
  ...
  const completedSlugs = React.useMemo(() => {
    ...
    return new Set(
      user.progress
        .filter(p => p.status === 'COMPLETED' && p.exercise?.slug)
        .map(p => p.exercise!.slug) // Non-null assertion for slug due to filter
    );
  ...
  return (
    <div className="bg-codedex-darkNavy min-h-screen pb-16">
      {/* Course Header - This is static for now */}
      <CourseHeader
      <CourseHeader
        title="Python"
      ...
      <div className="codedex-container grid grid-cols-1 lg:grid-cols-12 gap-8 pt-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          {isLoading ? (
            // Display skeletons for chapters while loading
            courseChaptersData.map((chapter) => (
              <Card key={chapter.number} className="bg-codedex-navy border-codedex-gold/10">
                <CardContent className="p-6">
                  <Skeleton className="h-6 w-1/2 mb-2" /> {/* Chapter Title */}
                  <Skeleton className="h-4 w-full mb-4" /> {/* Chapter Description */}
            ...
            courseChaptersData.map((chapter) => {
              const exercisesWithCompletion = chapter.exercises.map(ex => {
                // Check if 'slug' exists and is not undefined
                if ('slug' in ex && ex.slug) {
                  return { ...ex, isCompleted: completedSlugs.has(ex.slug) };
                }
                return ex; // Return exercise as is if no slug (e.g. bonus/challenge placeholders)
              });
              return (
                <CourseChapter
                  key={chapter.number}
                  ...
                  isClubOnly={chapter.isClubOnly}
                  isFinalProject={chapter.isFinalProject}
                  isCertificate={chapter.isCertificate}
        ...
        <div className="lg:col-span-4 space-y-8">
          <CourseSidebar
            level={1} // Keep level as a prop for now, can be dynamic later
            // username and progress props are removed
            cheatSheets={[
