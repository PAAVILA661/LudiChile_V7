...
const AdminEditStaticPage: React.FC<AdminPageProps> = ({
  user: _adminUser,
}) => {
  ...
  const fetchPageData = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 404) {
        ...
        setFormData({
          title: PREDEFINED_TITLES[pageSlug] || "New Page",
          content: "",
        });
      } else if (!response.ok) {
    ...
    <div className="min-h-screen bg-codedex-darkNavy text-white p-8">
      <div className="codedex-container">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-3xl font-pixel text-codedex-gold">
              Edit Page: {formData.title || pageSlug}
            </h1>
          </div>
        </div>
        ...
        <form
          ...
          className="bg-codedex-navy p-6 rounded-lg shadow-lg space-y-6"
        >
          <div>
            <label
              htmlFor="title"
