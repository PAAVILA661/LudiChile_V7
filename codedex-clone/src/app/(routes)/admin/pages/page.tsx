...
const PREDEFINED_PAGES: Array<{
...
}> = [
  {
    slug: "about",
    title: "About Us",
    description: "Content about your platform.",
  },
  {
    slug: "terms",
    title: "Terms of Service",
    description: "Your platform's terms and conditions.",
  },
  {
    slug: "privacy",
    title: "Privacy Policy",
    description: "Your platform's privacy policy.",
  },
  {
    slug: "faq",
    title: "FAQ",
...
const AdminStaticPagesOverviewPage: React.FC<AdminPageProps> = ({
  user: _adminUser,
}) => {
  ...
  return (
    <div className="min-h-screen bg-codedex-darkNavy text-white p-8">
      <div className="codedex-container">
        ...
        {isLoading && (
          <p className="text-center text-lg">Loading page information...</p>
        )}
        {error && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {PREDEFINED_PAGES.map((p) => {
              ...
              return (
                <div
                  key={p.slug}
                  className="bg-codedex-navy p-6 rounded-lg shadow-lg flex flex-col justify-between"
                >
                  <div>
                    <h2 className="text-xl font-semibold text-codedex-teal mb-2">
                      {dbInfo?.title || p.title}
                    </h2>
                    <p className="text-gray-400 text-sm mb-1">
                      Slug:{" "}
                      <code className="bg-codedex-darkNavy/50 px-1 py-0.5 rounded text-xs">
                        {p.slug}
                      </code>
                    </p>
                  ...
                  <div className="mt-auto">
                    <Button
                      className="w-full bg-codedex-gold/80 hover:bg-codedex-gold text-codedex-darkNavy"
                      asChild
                    >
                    ...
                    {dbInfo?.updated_at && (
                      <p className="text-xs text-gray-500 mt-2 text-right">
                        Last updated:{" "}
                        {new Date(dbInfo.updated_at).toLocaleDateString()}
                      </p>
                    )}
                  </div>
                </div>
