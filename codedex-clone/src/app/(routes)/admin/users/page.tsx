...
interface UserForAdminList extends Omit<User, 'user_xp' | 'progress' | 'user_badges'> {
...
const AdminUsersPage: React.FC<AdminPageProps> = ({ user: adminUser }) => {
  ...
  const [selectedUser, setSelectedUser] = useState<UserForAdminList | null>(null);
  ...
  const handleUpdateRole = async () => {
    ...
    try {
      ...
      setUsers(prevUsers =>
        prevUsers.map(u => u.id === selectedUser.id ? { ...u, role: data.role } : u)
      );
    ...
    <div className="min-h-screen bg-codedex-darkNavy text-white p-8">
      <div className="codedex-container">
        ...
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-pixel text-codedex-gold">User Management</h1>
          ...
          <div className="bg-codedex-navy shadow-xl rounded-lg overflow-hidden">
            <table className="min-w-full">
              ...
              <thead className="bg-codedex-darkNavy/50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-pixel text-codedex-teal uppercase tracking-wider">Name</th>
                  <th className="px-6 py-3 text-left text-xs font-pixel text-codedex-teal uppercase tracking-wider">Email</th>
                  <th className="px-6 py-3 text-left text-xs font-pixel text-codedex-teal uppercase tracking-wider">Role</th>
                  <th className="px-6 py-3 text-left text-xs font-pixel text-codedex-teal uppercase tracking-wider">Joined</th>
              ...
              <tbody className="divide-y divide-codedex-gold/10">
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-codedex-darkNavy/30 transition-colors duration-150">
      ...
      {selectedUser && (
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="bg-codedex-navy border-codedex-gold/20 text-white">
            <DialogHeader>
              <DialogTitle className="text-codedex-gold font-pixel">Edit Role for {selectedUser.email}</DialogTitle>
              <DialogDescription className="text-gray-400">
                Select the new role for this user.
                Be careful when changing roles, especially for admin users.
              </DialogDescription>
            </DialogHeader>

            <div className="py-4 space-y-2">
                <label htmlFor="role-select" className="text-sm font-medium text-gray-300 block mb-1">Role</label>
                <Select value={selectedRole} onValueChange={handleRoleChange}>
                    <SelectTrigger id="role-select" className="w-full bg-codedex-darkNavy border-gray-700 text-white">
                        <SelectValue placeholder="Select a role" />
                    </SelectTrigger>
                    <SelectContent className="bg-codedex-navy border-codedex-gold/20 text-white">
                        {Object.values(UserRole).map(role => (
                            <SelectItem key={role} value={role} className="hover:bg-codedex-teal/20 focus:bg-codedex-teal/20">
                                {role}
                            </SelectItem>
                        ))}
                    </SelectContent>
            ...
            {updateError && <p className="text-red-400 text-xs mt-2">Error: {updateError}</p>}

            <DialogFooter className="mt-4">
              <DialogClose asChild>
                <Button variant="outline" className="border-gray-700 text-gray-300 hover:bg-gray-700/50">Cancel</Button>
              </DialogClose>
              <Button
