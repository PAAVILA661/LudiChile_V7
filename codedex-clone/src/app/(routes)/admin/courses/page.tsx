...
const AdminCoursesPage: React.FC<AdminPageProps> = ({ user: _adminUser }) => {
  ...
  const [currentCourse, setCurrentCourse] = useState<CourseFormData | null>(
    null,
  );
  ...
  const openEditModal = async (course: CourseForAdminList) => {
    ...
    try {
      ...
      setFormError((err as Error).message || "Could not load course data.");
    } finally {
  ...
  const handleDeleteCourse = async (courseId: string) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this course? This may affect related chapters and exercises.",
      )
    )
      return;
    setIsSubmitting(true);
                ...
                {courses.map((course) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2 flex items-center">
                      <Button
                        variant="link"
                        className="text-sky-400 hover:text-sky-400/80 p-0 h-auto"
                        asChild
                      >
                        <Link
                          href={`/admin/courses/${course.id}/chapters`}
                          title="Manage Chapters & Exercises"
                        >
