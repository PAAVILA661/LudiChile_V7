...
import type { ChapterForAdminList, ChapterFormData } from '@/types/chapterAdmin';
...
import { ArrowLeft, PlusCircle, Edit, Trash2, BookText, ListOrdered } from 'lucide-react';

const AdminCourseChaptersPage: React.FC<AdminPageProps> = ({ user: adminUser }) => {
  ...
  const [currentChapter, setCurrentChapter] = useState<ChapterFormData | null>(null);
  ...
  const fetchCourseDetails = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message); // Podría ser un error diferente para detalles del curso
    }
  }, [courseId]);
  ...
  useEffect(() => {
    fetchCourseDetails();
    fetchChapters();
  }, [courseId, fetchCourseDetails, fetchChapters]);

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    const val = name === 'order' ? parseInt(value, 10) || 0 : value;
  ...
  const openCreateModal = () => {
    ...
    setFormData({ title: '', slug: '', description: '', order: chapters.length + 1 });
    setFormError(null);
    setIsModalOpen(true);
  };
  ...
  const openEditModal = async (chapter: ChapterForAdminList) => {
    ...
    try{
        const res = await fetch(`/api/admin/courses/${courseId}/chapters/${chapter.id}`);
        if(!res.ok) throw new Error('Failed to fetch chapter details for editing');
  ...
  const handleDeleteChapter = async (chapterId: string) => {
    if (!window.confirm('Are you sure you want to delete this chapter? All exercises within it will also be affected (potentially deleted if cascade is on).')) return;
    setIsSubmitting(true);
    setFormError(null);
    try {
      const response = await fetch(`/api/admin/courses/${courseId}/chapters/${chapterId}`, { method: 'DELETE' });
  ...
  if (isLoading && !course) return <div className="min-h-screen bg-codedex-darkNavy flex items-center justify-center text-white text-xl">Loading course content...</div>;
  if (error && !course) return <div className="min-h-screen bg-codedex-darkNavy flex items-center justify-center text-red-500 text-xl">Error loading course: {error}</div>;
    ...
    <div className="min-h-screen bg-codedex-darkNavy text-white p-8">
      <div className="codedex-container">
        <div className="flex items-center justify-between mb-6">
          <div>
            <Button variant="ghost" size="sm" className="text-gray-400 hover:text-gray-200 mb-2" onClick={() => router.push('/admin/courses')}>
                <ArrowLeft size={16} className="mr-2" /> Back to Courses
            </Button>
            <h1 className="text-3xl font-pixel text-codedex-gold">Manage Chapters</h1>
            {course && <p className="text-codedex-teal">Course: {course.title}</p>}
          </div>
        ...
        {isLoading && chapters.length === 0 && <p className="text-center text-lg">Loading chapters...</p>}
        {error && chapters.length === 0 && <p className="text-center text-red-500 text-lg">Error: {error}</p>}
          ...
          <div className="bg-codedex-navy shadow-xl rounded-lg overflow-hidden">
            <table className="min-w-full">
              <thead className="bg-codedex-darkNavy/50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-pixel text-codedex-teal uppercase tracking-wider">Order</th>
              ...
              <tbody className="divide-y divide-codedex-gold/10">
                {chapters.map((chapter) => (
                  <tr key={chapter.id} className="hover:bg-codedex-darkNavy/30 transition-colors duration-150">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300 w-16 text-center">{chapter.order}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{chapter.title}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{chapter.slug}</td>
                    {/* <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-400">{chapter._count?.exercises || 0}</td> */}
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2 flex items-center">
                      <Button variant="link" className="text-blue-400 hover:text-blue-400/80 p-0 h-auto" asChild>
                        <Link href={`/admin/courses/${courseId}/chapters/${chapter.id}/exercises`} title="Manage Exercises">
                          <ListOrdered size={14} className="mr-1" /> Exercises
                        </Link>
                      </Button>
                      <Button variant="link" className="text-codedex-gold hover:text-codedex-gold/80 p-0 h-auto" onClick={() => openEditModal(chapter)} title="Edit Chapter">
                        <Edit size={14} className="mr-1" /> Edit
                      </Button>
                      <Button variant="link" className="text-red-500 hover:text-red-500/80 p-0 h-auto" onClick={() => handleDeleteChapter(chapter.id)} title="Delete Chapter">
                ...
                {chapters.length === 0 && !isLoading && (
                    <tr><td colSpan={4} className="text-center py-10 text-gray-500">No chapters found for this course.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <DialogContent className="bg-codedex-navy border-codedex-gold/20 text-white sm:max-w-[500px]">
            <DialogHeader>
              ...
              <DialogDescription className="text-gray-400">
                {modalMode === 'create' ? `For course: ${course?.title}` : `Editing: ${currentChapter?.title}`}
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4 py-4">
              ...
              <div>
                ...
                <Input id="order" name="order" type="number" value={formData.order === undefined ? '' : formData.order} onChange={handleInputChange} required className="bg-codedex-darkNavy border-gray-700" />
              </div>
              <div>
              ...
              <DialogFooter className="mt-6">
                ...
                <Button type="submit" disabled={isSubmitting} className="bg-codedex-teal text-white hover:bg-codedex-teal/80">
                  {isSubmitting ? 'Saving...' : (modalMode === 'create' ? 'Create Chapter' : 'Save Changes')}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
