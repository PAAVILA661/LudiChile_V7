...
export async function generateStaticParams() {
  try {
    ...
    const allSlugs = Array.from(new Set([...pages.map(p => p.slug), ...predefinedSlugs]));
...
async function getPageContent(slug: string) {
  try {
    ...
    console.error(`Failed to fetch page content for slug [${slug}] from DB:`, error);
...
export default async function StaticContentPage({ params }: StaticPageProps) {
    ...
    if (predefinedSlugs.includes(params.slug)) {
        ...
        return (
            <div className="codedex-container text-white py-12 px-4">
                <h1 className="text-3xl font-pixel text-codedex-gold mb-6 capitalize">{params.slug.replace('-',' ')}</h1>
                <div className="prose prose-invert prose-lg max-w-none bg-codedex-navy p-6 rounded-md">
                    <p>This page (<code>{params.slug}</code>) has not been configured yet.</p>
  ...
  return (
    <div className="codedex-container text-white py-12 px-4">
      <h1 className="text-3xl font-pixel text-codedex-gold mb-6">{page.title}</h1>
