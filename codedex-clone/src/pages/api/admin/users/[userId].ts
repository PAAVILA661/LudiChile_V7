...
async function handler(
  ...
  if (!role || !Object.values(UserRole).includes(role as UserRole)) {
    return res.status(400).json({ message: `Invalid role specified. Valid roles are: ${Object.values(UserRole).join(', ')}` });
  }
  ...
  if (userId === req.adminUser.userId && role !== UserRole.ADMIN) {
     ...
     if (adminCount <= 1) {
        return res.status(403).json({ message: 'Cannot remove the last admin role.' });
     }
  }
  ...
  try {
    ...
    const updatedUser = await prisma.user.update({
      ...
      select: { // Seleccionar campos para devolver, excluyendo password_hash
        id: true,
        name: true,
        email: true,
        role: true,
    ...
    return res.status(500).json({ message: 'Internal Server Error while updating user role' });
  }
}
