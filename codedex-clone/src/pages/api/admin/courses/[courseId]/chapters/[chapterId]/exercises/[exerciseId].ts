...
async function handler(req: AuthenticatedAdminRequest, res: NextApiResponse) {
  ...
  if (typeof courseId !== 'string' || typeof chapterId !== 'string' || typeof exerciseId !== 'string') {
    return res.status(400).json({ message: 'Valid Course ID, Chapter ID, and Exercise ID are required' });
  }
  ...
  if (req.method === 'PUT') {
    const { title, slug, description, instructions, initial_code, expected_output, xp_value, order } = req.body;
    if (!title || !slug || order === undefined || xp_value === undefined) {
      return res.status(400).json({ message: 'Title, Slug, Order, and XP Value are required' });
    }
    try {
      ...
      if (existingExerciseBySlug) {
        return res.status(409).json({ message: 'Another exercise with this slug already exists in this chapter' });
      }
      ...
      const updatedExercise = await prisma.exercise.update({
        ...
        data: {
          ...
          initial_code,
          expected_output,
          xp_value: parseInt(xp_value as string, 10),
