...
async function handler(req: AuthenticatedAdminRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      console.error(`Failed to fetch exercises for chapter ${chapterId}:`, error);
  ...
  if (req.method === 'POST') {
    ...
    if (!title || !slug || order === undefined || xp_value === undefined) {
      return res.status(400).json({ message: 'Title, Slug, Order, and XP Value are required for an exercise' });
    }
    ...
    try {
      ...
      if (existingExercise) {
        return res.status(409).json({ message: 'Exercise with this slug already exists in this chapter' });
      }
      ...
      console.error(`Failed to create exercise for chapter ${chapterId}:`, error);
      return res.status(500).json({ message: 'Internal Server Error' });
    }
  }
